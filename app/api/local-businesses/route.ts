import { NextRequest, NextResponse } from 'next/server';
import { z } from 'zod';
import { d1, UserRecord } from '@/lib/d1';
import { getCurrentSession } from '@/lib/auth/session';
import { apiRateLimiter } from '@/lib/kv';
import { searchLocalBusinesses } from '@/lib/providers/google-maps';
import { deductCredits, getCreditsBalance } from '@/lib/services/credits';

export const dynamic = 'force-dynamic';

const localSearchSchema = z.object({
  category: z.string().min(1, 'Category/keyword is required'),
  city: z.string().optional(),
  areaOrPincode: z.string().optional(),
  country: z.string().default('US'),
  requirePhone: z.boolean().default(false),
  requireWebsite: z.boolean().default(false),
  minRating: z.number().optional(),
});

export async function POST(req: NextRequest) {
  try {
    const apiKeyHeader =
      req.headers.get('x-api-key') ||
      req.headers.get('authorization')?.replace(/^Bearer\s+/i, '');
    let user: UserRecord | null = null;

    if (apiKeyHeader) {
      user = await d1
        .prepare('SELECT * FROM users WHERE api_key = ?')
        .bind(apiKeyHeader)
        .first<UserRecord>();
    } else {
      const session = await getCurrentSession();
      if (session) {
        user = await d1
          .prepare('SELECT * FROM users WHERE id = ?')
          .bind(session.userId)
          .first<UserRecord>();
      }
    }

    if (!user) {
      return NextResponse.json({ error: 'Unauthorized. Invalid API key or session.' }, { status: 401 });
    }

    const { success: rateLimitSuccess } = await apiRateLimiter.limit(user.id);
    if (!rateLimitSuccess) {
      return NextResponse.json({ error: 'Too many requests. Please slow down.' }, { status: 429 });
    }

    const body = await req.json();
    const validation = localSearchSchema.safeParse(body);
    if (!validation.success) {
      return NextResponse.json({ error: 'Invalid request data', details: validation.error.format() }, { status: 400 });
    }

    // Check if user has saved a custom Google Maps API key in their settings
    const customKeyRecord = await d1
      .prepare('SELECT api_key FROM user_provider_keys WHERE user_id = ? AND provider = "google_maps" AND enabled = 1')
      .bind(user.id)
      .first<{ api_key: string }>();

    const businesses = await searchLocalBusinesses(validation.data, customKeyRecord?.api_key);

    // Deduct 1 credit for the search batch
    if (businesses.length > 0) {
      await deductCredits(user.id, 1);
    }

    const creditsRemaining = await getCreditsBalance(user.id);

    return NextResponse.json({
      success: true,
      results: businesses,
      count: businesses.length,
      creditsRemaining,
    });
  } catch (error: any) {
    console.error('Local business search error:', error);
    return NextResponse.json({ error: error.message || 'Failed to search local businesses' }, { status: 500 });
  }
}
